...
Мы готовы двигаться дальше и можем перенести код который отправляет данные с микроконтроллера 
по MQTT и принять их на другом микроконтроллере. Мы будем использовать ESP8266 так как он обладает
встроенным WiFi и можем подключить его к интернету через любой роутер (в том чиле раздав интернет 
с телефона)